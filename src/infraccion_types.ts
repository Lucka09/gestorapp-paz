...
// ─── NUEVO: Cotización CABA (opcional, para presupuestos multi-jurisdicción) ─
cotizacionCABA?: CotizacionCABA
...
// ═══════════════════════════════════════════════════════════════════════════════
// ─── CONSULTA DE INFRACCIONES — PORTAL CABA (buenosaires.gob.ar) ─────────────
//
// Portal CABA tiene dos pasos:
//   Paso 1: Consulta de infracciones → Monto 1 (total sin tachar)
//   Paso 2: Comprobante de legajo (solo si hay puntos rojos) → Monto 2 (base × 2.25)
//
// Reglas de negocio:
//   - Deuda CABA = Monto 1 + Monto 2
//   - % que abona el cliente: 70% si deuda ≤ 300.000, 67% si > 300.000
//   - SUAT único global (no por jurisdicción)

export interface ActaCABA {
id: string              // generado: `caba_${nroActa}_${timestamp}`
nroActa: string
importeBase: number     // monto que figura en el portal (sin multiplicador)
esPuntosRojos: boolean  // requiere paso 2 (comprobante de legajo)
fecha?: string
estado?: string
}

export interface CotizacionCABA {
actas: ActaCABA[]
cantidad: number
tienePuntosRojos: boolean
monto1: number                              // total actas normales (o total sitio)
monto2: number                              // (total puntos rojos base) × 2.25
deudaTotal: number                          // monto1 + monto2
porcentajePagoCliente: number               // 0.70 o 0.67
montoACobrarAlCliente: number               // deudaTotal × porcentaje
umbralAplicado: number                      // 300000 (para trazabilidad)
}