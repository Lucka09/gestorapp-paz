...
function SupervisionPremiosView() {
  const { data: asesores, isLoading } = usePremiosEquipo()
  const { mesActual } = useCierreMensual()
  const navigate = useNavigate()
  const periodoLabel = `${MESES[mesActual.mes]} ${mesActual.anio}`
  const totalAPagar = (asesores ?? []).reduce(
    (s, a) => s + a.data.premiosA_pesos + a.data.premiosB_pesos, 0,
  )

  if (isLoading) return <Spinner label="Cargando premios del equipo..." />
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Avance de objetivos de cada empleado con premio asignado</p>
          </div>
        </div>
        <button
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm flex items-center gap-4">
        ...
        <div>
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">
            Total a pagar en premios este mes
          </p>
          <p className="text-3xl font-extrabold"
             style={{ fontFamily: 'var(--font-display)', color: totalAPagar > 0 ? '#D4621A' : '#111827' }}>
            {formatPesos(totalAPagar)}
          </p>
          <p className="text-xs text-gray-400 mt-0.5">
            {asesores?.length ?? 0} empleado{(asesores?.length ?? 0) !== 1 ? 's' : ''} con premio asignado
          </p>
        </div>
      </div>
      ...
      {!asesores || asesores.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-gray-300">
          ...
          <p className="text-base font-semibold text-gray-400">Sin empleados con premio asignado</p>
          <p className="text-sm text-gray-300 mt-1 text-center max-w-sm">
            El sistema de premios aplica al rol Asesor Comercial. Asigná ese rol desde Equipo.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {asesores.map(a => <AsesorCard key={a.uid} asesor={a} />)}
        </div>
      )}
      ...
      <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-2xl px-5 py-4">
        ...
        <p className="text-sm text-blue-800 leading-relaxed">
          Vista de solo lectura. Los premios por trámites se calculan automáticamente al registrar el pago.
          Los premios por hitos de facturación los acreditás manualmente desde Configuración → Premios.
        </p>
      </div>
    </div>
  )
    ...
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm mb-4">
          ...
          <div className="relative mb-1">
            ...
            {hitosOrdenados.map(h => {
              ...
              return (
                <div key={h.id} className="absolute -top-1.5 -translate-x-1/2 text-lg"
                     style={{ left: `${pos}%`, filter: !reached ? 'grayscale(1) opacity(0.4)' : undefined, transition: 'all 0.3s ease' }}
      ...
      <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-2xl px-5 py-4">
        ...
        <p className="text-sm text-blue-800 leading-relaxed">
          ...
          Para consultas, contactar a la dirección de la gestoría.
        </p>
      </div>
