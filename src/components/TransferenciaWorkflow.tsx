  ...
  return (
    <div>
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-1 cursor-pointer transition-all min-h-[72px] ${
          preview
            ? 'border-emerald-300 bg-emerald-50/30'
            : 'border-gray-200 hover:border-[#D4621A]/40 hover:bg-[#D4621A]/5'
        }`}
      >
        {preview ? (
...
export default function TransferenciaWorkflow({ tramiteId }: Props) {
  ...
  const esAsesor = ['propietario', 'admin', 'admin_gral', 'asesor_comercial', 'vendedor', 'operador'].includes(
    user?.rol ?? '',
  )
  ...
  const docs2Completos = () => {
    const reqs = [
      ...
      'cedulaFrente',
      'cedulaDorso',
      'verificacionPolicialFrente',
      'verificacionPolicialDorso',
      'dniCompradorFrente',
      'dniCompradorDorso',
    ]
        ...
        <div>
          ...
          <p className="text-gray-400 text-xs mt-0.5">
            ...
            {workflow?.estadoWorkflow && (
              <span
                className={`ml-2 px-2 py-0.5 rounded-full text-[10px] font-bold ${ESTADO_TRF_COLORS[workflow.estadoWorkflow]}`}
              >
          ...
          {pasoActual === 1 ? (
            <>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={e => setP1(p => ({ ...p, observacion: e.target.value }))}
                  rows={2}
            ...
            <Resumen
              filas={[
                {
                  l: 'Futura radicación',
                  v: workflow?.paso1?.futuraRadicacion
                    ? `Sí — ${workflow.paso1.jurisdiccionDestino ?? 'sin especificar'}`
                    : 'No',
                },
            ...
            <>
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-start gap-2">
                ...
                <p className="text-xs text-blue-700">
                  ...
                  Si falta algún documento, dejá la observación abajo explicando por qué, para tener
                  registro y poder completarlo después.
                </p>
              </div>

              <DocParUploader
                label="Formulario 08 / 08 Digital"
                onFrente={setF2('formulario08Frente')}
                onDorso={setF2('formulario08Dorso')}
                prevFrente={prev2['formulario08Frente']}
                prevDorso={prev2['formulario08Dorso']}
              />

              <DocParUploader
                label="Título del vehículo"
                onFrente={setF2('tituloFrente')}
                onDorso={setF2('tituloDorso')}
                prevFrente={prev2['tituloFrente']}
                prevDorso={prev2['tituloDorso']}
              />

              <DocParUploader
                label="Cédula verde"
                onFrente={setF2('cedulaFrente')}
                onDorso={setF2('cedulaDorso')}
                prevFrente={prev2['cedulaFrente']}
                prevDorso={prev2['cedulaDorso']}
              />

              <DocParUploader
                label="Verificación policial"
                onFrente={setF2('verificacionPolicialFrente')}
                onDorso={setF2('verificacionPolicialDorso')}
                prevFrente={prev2['verificacionPolicialFrente']}
                prevDorso={prev2['verificacionPolicialDorso']}
              />

              <DocParUploader
                label="DNI del comprador"
                onFrente={setF2('dniCompradorFrente')}
                onDorso={setF2('dniCompradorDorso')}
                prevFrente={prev2['dniCompradorFrente']}
                prevDorso={prev2['dniCompradorDorso']}
              />

              <DocParUploader
                label="Formulario 04 (radicación)"
                onFrente={setF2('formulario04Frente')}
                onDorso={setF2('formulario04Dorso')}
                prevFrente={prev2['formulario04Frente']}
                prevDorso={prev2['formulario04Dorso']}
              />

              {!workflow?.gestorId && (
                <div>
                  <label className="text-xs font-semibold text-gray-500 mb-1 block">
                    Asignar gestor para la gestión en registro (opcional)
                  </label>
                  <select
                    onChange={e => {
                      const g = gestoresEquipo.find((g: any) => g.uid === e.target.value)
              ...
              <textarea
                ...
                rows={2}
                placeholder="Observaciones — ej: 'falta DNI comprador, cliente lo trae la semana próxima'..."
                className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm outline-none resize-none"
              />
          ...
          {pasoActual === 3 && esGestor ? (
            <>
              <p className="text-xs text-gray-500">
                El registro suele emitir 3 recibos. Recomendado cargarlos, pero si no los tenés a mano todavía,
                podés avanzar igual y dejarlo aclarado abajo.
              </p>
              ...
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <FotoUploader
                    label="Frente"
                    onChange={setF3('reciboTrfFrente')}
                    preview={prev3['reciboTrfFrente']}
                  />
                  <FotoUploader
                    label="Dorso (opcional)"
                    onChange={setF3('reciboTrfDorso')}
                    preview={prev3['reciboTrfDorso']}
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <FotoUploader
                    label="Frente"
                    onChange={setF3('reciboArbaFrente')}
                    preview={prev3['reciboArbaFrente']}
                  />
                  <FotoUploader
                    label="Dorso (opcional)"
                    onChange={setF3('reciboArbaDorso')}
                    preview={prev3['reciboArbaDorso']}
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <FotoUploader
                    label="Frente"
                    onChange={setF3('reciboSuatsFrente')}
                    preview={prev3['reciboSuatsFrente']}
                  />
                  <FotoUploader
                    label="Dorso (opcional)"
                    onChange={setF3('reciboSuatsDorso')}
                    preview={prev3['reciboSuatsDorso']}
                  />
                </div>
              </div>
              ...
              {(!files3['reciboTrfFrente'] || !files3['reciboArbaFrente'] || !files3['reciboSuatsFrente']) && (
                <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700">
                  ...
                  <span>
                    Recomendado subir los 3 recibos. Si falta alguno, dejalo aclarado en observaciones — no
                    impide avanzar.
                  </span>
                </div>
              )}
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  confirmarPaso3(
                    {
                      montoRegistro: monto3,
                      notaMontoRegistro: notaMonto,
                      observacion: obs3,
                      reciboTransferencia: null as any,
                      reciboArba: null as any,
                      reciboSuats: null as any,
                    } as any,
                    files3 as any,
                    geoData,
                  )
            ...
            <Resumen
              filas={[
                {
                  l: 'Monto abonado',
                  v: `$${workflow?.paso3?.montoRegistro?.toLocaleString('es-AR') ?? 0}`,
                },
                {
                  l: 'Geo',
                  v: workflow?.paso3?.geoPresencia
                    ? `${workflow.paso3.geoPresencia.lat.toFixed(4)}, ${workflow.paso3.geoPresencia.lng.toFixed(4)}`
                    : '—',
                },
              ]}
            />
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 space-y-4">
          {pasoActual === 4 && (
            <>
              <div
                className={`p-3 rounded-xl border text-xs ${
                  futuraRad ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-blue-50 border-blue-200 text-blue-700'
                }`}
              >
                <div className="flex items-center gap-1 font-semibold mb-1">
                  <Clock size={11} /> Seguimiento activo
                </div>
                {configPlazos.label} · El sistema genera un recordatorio interno cada {configPlazos.frecuenciaAlertaDias}{' '}
                días — es solo informativo.
                {workflow?.recordatorioSeguimiento && (
                  <div className="mt-1">
                    Próximo recordatorio:{' '}
                    <strong>{workflow.recordatorioSeguimiento.toDate().toLocaleDateString('es-AR')}</strong>
                  </div>
                )}
              </div>
              ...
              {reciboListo && (
                <button
                  type="button"
                  onClick={() => confirmarReciboListo({} as any)}
                  disabled={guardando}
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 space-y-4">
          {pasoActual === 5 ? (
            <>
              ...
              <textarea
                value={p5.observacion}
                onChange={e => setP5(p => ({ ...p, observacion: e.target.value }))}
                rows={2}
          ...
          {pasoActual === 6 && esGestor ? (
            <>
              <div className="bg-purple-50 border border-purple-200 rounded-xl p-3 flex items-start gap-2">
                ...
                <div>
                  <p className="text-xs text-purple-700 font-semibold mb-0.5">Presencia en el registro</p>
              ...
              {geo.ubicacion && (
                <div className="flex items-center gap-2 text-xs text-emerald-600 bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2">
                  <CheckCircle2 size={12} /> Ubicación capturada:{' '}
                  {geo.ubicacion.direccionAprox ?? `${geo.ubicacion.lat.toFixed(4)}, ${geo.ubicacion.lng.toFixed(4)}`}
                </div>
              )}
              ...
              <div className="space-y-1">
                ...
                <FotoUploader
                  ...
                  onChange={f => {
                    setFile6(f)
                    setPrev6(f ? URL.createObjectURL(f) : null)
                  }}
                  preview={prev6}
                />
              </div>
              ...
              {!file6 && (
                <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700">
                  <AlertTriangle size={12} className="mt-0.5 shrink-0" />
                  <span>
                    Recomendado adjuntar la foto del recibo físico. Si no es posible, dejalo aclarado arriba — no
                    impide confirmar el retiro.
                  </span>
                </div>
              )}
            ...
            <Resumen
              filas={[
                { l: 'Presentado', v: workflow?.paso6?.presentadoEnRegistro ? '✅ Sí' : '—' },
                {
                  l: 'Geo',
                  v:
                    workflow?.paso6?.geoRetiro?.direccionAprox ??
                    `${workflow?.paso6?.geoRetiro?.lat?.toFixed(4) ?? '—'}`,
                },
              ]}
            />
          ) : (
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 space-y-4">
          {pasoActual === 7 ? (
            <>
              ...
              <FotoUploader label="Foto constancia de entrega (opcional)" onChange={f => setFile7(f)} />

              <textarea
                value={p7.observacionFinal}
                onChange={e => setP7(p => ({ ...p, observacionFinal: e.target.value }))}
                rows={2}
