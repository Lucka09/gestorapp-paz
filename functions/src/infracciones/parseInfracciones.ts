...
export interface ConfigCotizacionMultas {
  ...
  overrides: {
    ...
    excluirSiSinDI:        boolean   // debeDI=false → excluir (sin Deber de Informar)
  }
...
export function clasificarActa(
  ...
  const base = config.matrizEstados[estado] ?? config.reglaPorDefecto
...
export function parseActa(
  ...
  return {
    ...
    detalles:            (raw.infracciones ?? []).map((d: { articulo: string; descripcion: string }) => ({
      articulo:    d.articulo,
...
export function parseRespuestaPortal(
  ...
  return raw.infracciones.map((a: RawActa) => parseActa(a, config))
}
