...
async function sendMediaMessage(to, tipo, link, caption, phoneNumberId, filename) {
    var _a, _b, _c;
    const emisor = resolverEmisor(phoneNumberId);
    const token = getMetaToken();
    const url = `${META_API_BASE}/${emisor}/messages`;
    const media = { link };
    if (caption && tipo !== 'audio')
        media.caption = caption;
    if (filename && tipo === 'document')
    ...
    try {
        ...
        const detalle = describirErrorMeta(error);
        console.error('[WA] Error enviando media:', detalle);
        throw new Error(`[WA] Error enviando media: ${detalle}`);
    }
    return (_c = (_b = (_a = data === null || data === void 0 ? void 0 : data.messages) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.id) !== null && _c !== void 0 ? _c : `local_${Date.now()}`;
}
